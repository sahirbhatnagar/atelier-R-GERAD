...
%\end{itemize}
%\pause 
\ctable[caption={Les principales objets en R et leurs nom de fonction},label=tab:table1,pos=h!,doinside=\small]{ccccc}{\tnote{tous les éléments doivent être du même type}\tnote[b]{les éléments peuvent être de différent types}
}{
    																\FL
...
\begin{frame}[fragile]{Importer et enregistrer des donnés et du code}



\ctable[caption={Les fonctions de bases pour importer et enregistrer des donnés et du code R},label=tab:table1,pos=h!,doinside=\small]{lccc}{\tnote{chaque valeur est séparée par un espace}\tnote[b]{chaque valeur est séparée par une virgule}\tnote[c]{objets dans ton espace de travail (Environment)}
}{
    																\FL
...
\item Ceci est un exemple qui démontre pourquoi R est un langage orientée objet
\end{itemize}
\pause 
\begin{knitrout}
\definecolor{shadecolor}{rgb}{1, 1, 1}\color{fgcolor}\begin{kframe}
...
\begin{itemize}
  \setlength\itemsep{2em}
  \item Utile pour voir la distribution de tes données
\item Pour les données continues et univariés
\end{itemize}
...
\begin{itemize}
  \setlength\itemsep{2em}
\item Utile pour voir la différence d'un variable continu entre plusieurs groupes
\end{itemize}

\begin{knitrout}
\definecolor{shadecolor}{rgb}{1, 1, 1}\color{fgcolor}\begin{kframe}
\begin{rcode}\label{unnamed-chunk-34}\begin{alltt}
\hlcom{# visualiser la différence de la taille entre les }
\hlcom{# hommes et les femme dans le jeu}
\hlcom{# de données 'survey' du library(MASS)}
