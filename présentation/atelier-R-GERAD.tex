...
\begin{frame}[fragile]{La fonction}
...
\hlcom{# Scatter plot des valeurs d'un vecteur contre leurs indices}
\hlcom{# équivalent à plot(mtcars[,"mpg"])}
\hlkwd{plot}\hlstd{(mtcars}\hlopt{$}\hlstd{mpg)}

\hlcom{# Graphique des couples (x, y)}
\hlkwd{plot}\hlstd{(mtcars}\hlopt{$}\hlstd{mpg, mtcars}\hlopt{$}\hlstd{disp)}
\end{alltt}
\end{rcode}\end{kframe}
\end{knitrout}
